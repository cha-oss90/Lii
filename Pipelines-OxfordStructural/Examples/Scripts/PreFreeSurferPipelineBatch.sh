...
get_batch_options() {
  ...
  StudyFolder="/project/2422119.01/data/" # Location of subject folders (named by subject IDs in SubjList)
...
main()
{
  ...
  for Subject in ${SubjList//@/ } ; do
    ...
    numT1ws=`ls ${StudyFolder}/${Subject}/struct | grep T1w_MPR.$ | wc -l`
    ...
    while [ $i -le $numT1ws ] ; do
      ...
      T1wInputImages=`echo "${T1wInputImages}${StudyFolder}/${Subject}/struct/T1w_MPR${i}/${Subject}__T1w_MPR${i}.nii.gz@"`
      i=$(($i+1))
    done

    # Detect Number of T2w Images
    #numT2ws=`ls ${StudyFolder}/${Subject}/unprocessed/${Scanner} | grep T2w_SPC.$ | wc -l`
    numT2ws=`ls ${StudyFolder}/${Subject}/struct/ | grep T2w_SPC.$ | wc -l`
